...
class HomeController extends Controller
{
    function index()
    {
        if (\Auth::check()) {
            ...
            $customers_data = Customer::get_customers($user_id);

            $topSaleCustomer=Sale::topSalesCustomers();
        $topSaleCustomerId=array();
            foreach ($topSaleCustomer as $key => $id) {
                $topSaleCustomer=get_object_vars($id);
                
                 array_push($topSaleCustomerId,$topSaleCustomer['customer_id']);
                 
                }
                $data['branch_id']   = 1;
            $data['cash_register_id'] =1;
                $salesArray1 = Sale::getSaleReportMonthlyChart($data);
        ...
        $branch_data= Branch::where('created_by', $user_id)->get();
        // $shiftData = ShiftSchedules::all();
        $users_data = User::where('parent_id', $user_id)->get();
   
$topFive =Sale::topSalesCustomers();
$topSell=SelledItems::getTopProduct();
