...
class PurchaseController extends Controller
{
    public function index()
    {
        if (Auth::user()->can('Manage Purchases')) {
            ...
            return view('purchases.index',compact('brands','cashregister','taxes','vendors'));
        } else {
    ...
    public function store(Request $request)
    {
        ...
        if (Auth::user()->can('Manage Purchases')) {
            ...
            $partial_paid   =$request->partial_paid != 0 ? $request->partial_paid : 0;
            ...
            if (isset($purchases) && !empty($purchases) && count($purchases) > 0) {
                ...
                if (count($result) > 0) {
                    ...
                    if($request->partial_paid !=0){
                        ...
                        $purchase->status = $request->paid_status;
                    }else{
                        $purchase->status = 0;
                    }
                    
                    $purchase->title           =  $request->purchase_title;
    ...
    public function previewPurchasedInvoice($template, $color)
    {
        ...
        return view('purchases.templates.' . $template, compact('purchase', 'preview', 'font_color', 'color', 'settings', 'user', 'vendordetails', 'img','logo','company_logo'));
    }
    ...
    {
         $user_id = Auth::user()->getCreatedBy();
         $taxes=Tax::where('created_by',  $user_id)->get();
         $branches=Branch::where('created_by',  $user_id)->get();
        $vendors =Vendor::where('created_by',  $user_id)->get();
        return view('purchases.create',compact('vendors', 'branches', 'taxes'));
    }
    ...
    public function import(Request $request)
    {
        ...
        $purchases = (new PurchaseImport())->toArray(request()->file('file'))[0];

        $totalCustomer = count($purchases) - 1;
        $errorArray    = [];
        for ($i = 1; $i <= count($purchases) - 1; $i++) {
            $pur =$purchases[$i];
            
            $vendor_id        = Vendor::vendor_id($pur[1]);
            $branch_id        = Branch::branch_id($pur[2]);
            $cash_register_id = CashRegister::cash_register_id($pur[3]);
            $invoice_id       = $this->invoicePurchaseNumber();
                ...
                $purchase->title           =  $pur[0];
                $purchase->created_by       = \Auth::user()->getCreatedBy();
